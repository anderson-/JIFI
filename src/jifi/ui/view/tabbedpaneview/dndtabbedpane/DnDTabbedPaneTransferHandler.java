 ...
 */
public class DnDTabbedPaneTransferHandler extends TransferHandler
        implements DropTargetListener {

    private static final long serialVersionUID = 1L;

    /**
     * The "data type" that starts and ends the drag-and-drops are instances of
     * <code>JTabbedPane</code>. While we're actually dnd'ing a "tab" and its
     * contents, the component both sending and receiving is a JTabbedPane.
     */
    ...
    /**
     * The location of the mouse cursor throughout the drag-and-drop. This is
     * here because of a deficiency in TransferHandler's design; you have no way
     * of knowing the exact drop location in the component with a plain
     * TransferHandler unless you implement DropTargetListener and get it that
     * way.
     */
     ...
     * Constructor.
     */
    public DnDTabbedPaneTransferHandler() {
        try {
    ...
    public void dragExit(DropTargetEvent e) {
        ...
        if (c instanceof DnDTabbedPane) {
            ((DnDTabbedPane) c).clearDnDIndicatorRect();
        }
    }
    ...
    public void dragOver(DropTargetDragEvent e) {
        ...
        if (destTabbedPane instanceof DnDTabbedPane) {

            ((DnDTabbedPane) c).dragOver(mouseLocation);
            ...
            Rectangle transferredTabBounds;
            if (t != null) {
                transferredTabBounds = t.getTabBounds();
            } else {
                transferredTabBounds = destTabbedPane.getBoundsAt(0);
//                
//                try {
//                    TabTransferable.TabTransferData transferData = (TabTransferable.TabTransferData) e.getTransferable().getTransferData(tabFlavor);
//                    transferredTabBounds = transferData.sourceTabbedPane.getBoundsAt(tab-1);
//                } catch (Exception ex) {
...
//                }
            ...
            ((DnDTabbedPane) c).setDnDIndicatorRect(
                    iBounds.x, iBounds.y,
    ...
    /**
     * Called when the drag-and-drop operation has just completed. This creates
     * a new tab identical to the one "dragged" and places it in the destination
        ...
        if (hasTabFlavor(t.getTransferDataFlavors()) && mouseLocation != null) {
            ...
            try {
                ...
                if (tabbedPane instanceof DnDTabbedPane) {
                    if (((DnDTabbedPane) tabbedPane).splitPanel()) {

                        Component it = c;
                        while (!(it instanceof JSplitPanel) && it != null) {
                            it = it.getParent();
                        }
                        if (it != null && it instanceof JSplitPanel) {
                            TabbedPaneView tabbedPaneView = ((JSplitPanel) it).getTabbedPaneView();

                            int orientation = ((DnDTabbedPane) tabbedPane).getSplitOrientation();

                            if (orientation == 1) {
                                tabbedPaneView.split(TabbedPaneView.VERTICAL, false);
                                tabbedPaneView.get(TabbedPaneView.TOP).getDnDTabbedPane().addTab(tabName, icon, comp, toolTip);
                            } else if (orientation == 2) {
                                tabbedPaneView.split(TabbedPaneView.VERTICAL, true);
                                tabbedPaneView.get(TabbedPaneView.BOTTOM).getDnDTabbedPane().addTab(tabName, icon, comp, toolTip);
                            } else if (orientation == 3) {
                                tabbedPaneView.split(TabbedPaneView.HORIZONTAL, false);
                                tabbedPaneView.get(TabbedPaneView.LEFT).getDnDTabbedPane().addTab(tabName, icon, comp, toolTip);
                            } else if (orientation == 4) {
                                tabbedPaneView.split(TabbedPaneView.HORIZONTAL, true);
                                tabbedPaneView.get(TabbedPaneView.RIGHT).getDnDTabbedPane().addTab(tabName, icon, comp, toolTip);
                            }
                        }
    ...
    /**
     * Selects the specified tab in the specified tabbed pane. This method can
     * be overridden by subclasses to do more stuff than simply select the tab.
     *
    ...
    class TabTransferable implements Transferable {
        ...
        public Rectangle getTabBounds() {
            return transferData.sourceTabbedPane.getBoundsAt(transferData.tabIndex);
        }
