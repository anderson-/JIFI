...
//                }
    ...
    private static final <T> T newInstance(Object ref) {
        if (ref instanceof Class) {
            ...
            if (Procedure.class.isAssignableFrom(c)) {
                try {
                    Procedure p = (Procedure) c.newInstance();
    ...
    private synchronized void ident() {
        try {
...
//            interpreter.setInterpreterState(Interpreter.STOP);
    ...
    @Override
    public synchronized void drawTopLayer(Graphics2D g, GraphicAttributes ga, InputState in) {
        ...
        if (in.isKeyPressed(KeyEvent.VK_CONTROL)) {
            if (!keyActionUsed) {
                if (in.isKeyPressed(KeyEvent.VK_Z)) {
                    hm.undo();
                    keyActionUsed = true;
                } else if (in.isKeyPressed(KeyEvent.VK_Y)) {
                    hm.redo();
                    keyActionUsed = true;
                } else if (in.isKeyPressed(KeyEvent.VK_X)) {
                    if (isValidSelection()) {
                        copy.clear();
                        hm.saveState();
                        redo.clear();
                    ...
                    keyActionUsed = true;
                } else if (in.isKeyPressed(KeyEvent.VK_C)) {
                    if (isValidSelection()) {
                        copy.clear();
                        for (Command c : selection) {
                            if (c instanceof Procedure) {
                    ...
                    keyActionUsed = true;
                } else if (in.isKeyPressed(KeyEvent.VK_V)) {
...
//        }
    ...
    public static void main(String[] args) {
        ...
        builder.setPluginManager(PluginManager.getInstance());
        builder.init();
        {
